...
def observation(agent, env):
    ...
    pos = agent.state.pos / torch.tensor([env.x_semidim, env.y_semidim], device=env.device)
    vel = agent.state.vel / torch.tensor([2 * env.x_semidim, 2 * env.y_semidim], device=env.device)
    ...
    return {"obs": obs, "pos": pos, "vel": vel} if env.use_gnn else obs
